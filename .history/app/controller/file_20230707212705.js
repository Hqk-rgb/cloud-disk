"use strict";constController=require("egg").Controller;constfs=require("fs");constpath=require("path");classFileControllerextendsController{
    // 上传
...
// 上传到ossletresult;try{result=awaitctx.oss.put(name,file.filepath);}catch(err){console.log(err);}console.log(result.url);// 写入到数据表if(result){letaddData={name:file.filename,ext:file.mimeType,md:result.name,file_id,user_id:currentUser.id,size:parseInt(s),isdir:0,url:result.url,};letres=awaitapp.model.File.create(addData);// 更新用户的网盘内存使用情况currentUser.used_size=currentUser.used_size+parseInt(s);currentUser.save();returnctx.apiSuccess(res);}ctx.apiFail("上传失败");}}module.exports=FileController;