...
class FileController extends Controller {
  ...
  async upload() {
    ...
    if (!ctx.request.files) {
      returnctx.apiFail("请先选择上传文件");
    }
    ctx.validate({
      file_id: { required: true, type: "int", defValue: 0, desc: "目录id" },
    });
    constfile_id = ctx.query.file_id;
    console.log(file_id + "<<<<<<<<<<");
    // 目录id是否存在
    if (file_id > 0) {
      // 目录是否存在
    ...
    // 根据file_id一直向上找到顶层目录
    constprefixPath = awaitservice.file.seachDir(file_id);
    ...
    if (currentUser.total_size - currentUser.used_size < s) {
      returnctx.apiFail("你的可用内存不足");
    }
    // 上传到oss
    letresult;
    try {
      result = awaitctx.oss.put(name, file.filepath);
    } catch (err) {
      console.log(err);
    }
    ...
    if (result) {
      ...
      letres = awaitapp.model.File.create(addData);
      // 更新用户的网盘内存使用情况
      currentUser.used_size = currentUser.used_size + parseInt(s);
      currentUser.save();
      returnctx.apiSuccess(res);
    }
    ctx.apiFail("上传失败");
  }
}
