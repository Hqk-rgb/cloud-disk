...
class FileController extends Controller{
    // 上传
    async upload(){const{ctx,app,service}=this;
    const currentUser=ctx.authUser;console.log(ctx.request.files);if(!ctx.request.files){returnctx.apiFail("请先选择上传文件");}ctx.validate({file_id:{required:true,type:"int",defValue:0,desc:"目录id",},});constfile_id=ctx.query.file_id;console.log(file_id+"<<<<<<<<<<");
    // 目录id是否存在
    if(file_id>0){
        // 目录是否存在
